#################################################
# HelloID-Conn-Prov-Target-Magister-Employee-Update
# PowerShell V2
# Version: 1.0.0
#################################################
...
try {
    # Verify if [actionContext.Data.ExternalId] has a value
    ...
    if ($actionContext.DryRun -eq $true) {
        Write-Verbose -Verbose "[DryRun] update Magister-Employee account for: [$($personContext.Person.DisplayName)], will be executed during enforcement"
    }
    # Process
    if (-not($actionContext.DryRun -eq $true)) {
        Write-Verbose 'update Magister-Employee account'

        # Make sure to test with special characters and if needed; add utf8 encoding.
        $splatParams = @{
            SessionToken               = $sessionToken
            PersoneelsNummer           = $actionContext.References.Account
            ...
            DatumUitDienst             = (Get-Date "2099-12-31")
        ...
        $outputContext.AuditLogs.Add([PSCustomObject]@{
                ...
                Message = "Update account with accountReference: [$($actionContext.References.Account)] was successful."
                IsError = $false
            })
    }
} catch {
    ...
    $ex = $PSItem
    $auditMessage = "Could not UpdateAccount Magister-Employee account. Error: $($ex.Exception.Message)"
